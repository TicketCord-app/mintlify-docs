...
<Info>
  Welcome messages and designs work on every plan. On Free, a small "Powered by TicketCord.com" line is added at the end of each welcome message. The **Self-help menu** and applications need **Basic** or higher, and your own **Footer** text inside a design needs **Enterprise**. To edit welcome messages you must be the bot owner, or hold a role granted dashboard access for the server.
</Info>

## How welcome messages fit together

- **Designs** are reusable messages built in the designer. A server can have up to 25.
- **Automatic messages** set which design each kind of ticket sends when it opens.
- **Per-button choices** override the default for one button, dropdown option, form, or application.

Each way of opening a ticket belongs to one ticket type, which is a row under **Automatic messages → When a ticket opens**:

| Row | Tickets it covers |
|-----|-------------------|
| **Button tickets** | A button or section button whose **When clicked** is **Ticket**, and tickets opened from an Auto Support thread |
| **Dropdown tickets** | A dropdown option whose **When clicked** is **Ticket** |
| **Modal tickets** | Every form: a button, dropdown option, or section button whose **When clicked** is **Form**, plus older form buttons and dropdown forms |
| **Application tickets** | Every application: a button, dropdown option, or section button whose **When clicked** is **Application** |
| **ModMail tickets** | Tickets opened by sending the bot a direct message |

<Note>
  Forms opened from a dropdown option use **Modal tickets**, not **Dropdown tickets**. **Dropdown tickets** only covers options that open a ticket straight away.
</Note>
...
<Steps>
  <Step title="Open the Messages tab">
    Go to **Dashboard → Bots**, pick your bot, choose the server, and open **Messages** under the **Messaging** group. The **Automatic messages** card lists every message TicketCord sends on its own, and **Message designs** below it is the library, with a counter out of 25.
  </Step>
...
<Steps>
  <Step title="Find the ticket type">
    In **Automatic messages**, keep **When a ticket opens** selected at the top of the card, and find the row: **Button tickets**, **Dropdown tickets**, **Modal tickets**, **Application tickets**, or **ModMail tickets**.
  </Step>
  <Step title="Pick the design">
    Choose a design in the row's picker. **TicketCord default** sends the built-in message, unless a button has a design of its own. **Create new design…** creates a design and sets it for this type.
  </Step>
  <Step title="Check the preview">
    Click the row to open it. It shows a preview of what the type sends, including the built-in message when the type is on **TicketCord default**, and the type's extra options. **Edit design** opens the chosen design in the designer.
  </Step>
  <Step title="Save">
...
1. The design picked in **Welcome message** on the button, dropdown option, form, or application that opened the ticket. This applies even when the ticket type is on **TicketCord default**.
2. The design (or **Legacy message (classic editor)**) picked for the ticket type under **Automatic messages**.
...
### Automatic messages: when a ticket opens

| Setting | Allowed values | Default |
|---------|----------------|---------|
| Design picker (one per ticket type row) | **TicketCord default**, or any design that is compatible with the ticket type. A type that still has a message from the old editor also offers **Legacy message (classic editor)**; open the row for a **Convert to a design** button | **TicketCord default** |
| **Show User Avatar** (**ModMail tickets** row) | on or off. Shows the member's avatar as a thumbnail beside the first text block | on |
| **Include form answers** (**Dropdown tickets** and **Modal tickets** rows), **Include application answers** (**Application tickets** row) | on or off. Only shown while the type sends a legacy message (classic or Components V2), and adds the answers the message does not already show. Designs and the built-in message ignore it | on |
| **Self-help menu** | on or off. Adds a dropdown of prepared answers to the welcome message, including the built-in one. **Basic** or higher | off |
| **Self-help menu placement** | **Inside container** or **Outside container**. Shown while the menu is on and the type sends a design | **Inside container** |
...
When a member picks an answer from the self-help menu, the **Response Message** is shown only to them.
...
<Steps>
  ...
  <Step title="Open the design">
    On the **Messages** tab, click **Edit** on the design's card, or open the ticket type's row under **Automatic messages** and click **Edit design**.
  </Step>
...
A form counts as using this design when the design is picked in its own **Welcome message**, or when the form has no design of its own and this design is picked for **Modal tickets** (or **Application tickets**, for applications) under **Automatic messages**.
...
- as the design for **Button tickets**, **Dropdown tickets**, or **ModMail tickets**,
...
<Info>
  The **Assistant** needs **Pro** or **Enterprise** on the bot owner's account, including the free trial. Each reply uses the bot owner's AI tokens, also when a staff member asks. On other plans the panel shows an upgrade notice instead of the chat. See [AI tokens](/billing/plans#ai-tokens).
</Info>
...
3. The welcome message is posted. Staff pings configured for the ticket appear above it, then your containers, the control buttons, the self-help menu, and the **AI Help** button when [AI Help for Staff](/ai/atlas) is on. ModMail tickets also get **Alert**, **Subscribe**, and **Mute** buttons. On Free, a "Powered by TicketCord.com" line is added at the bottom.
...
| Symptom | Cause | Fix |
|---------|-------|-----|
| The built-in message is sent instead of your design | The ticket type is on **TicketCord default**, the change was not saved, or you picked the design on the wrong row | Pick the design on the right row and click **Save changes**. Forms use **Modal tickets** |
...
| A design shows "(incompatible variables)" and cannot be picked | It uses variables that have no value for that ticket type, such as a form answer variable on **Button tickets** | Edit the design, or use a separate design there |
...
In **Panels & Embeds**, click the button and pick a design in **Welcome message**. For a form or application, it is under **Routing**. It overrides the ticket type's choice, even when that is **TicketCord default**.

**Do I need a paid plan for custom welcome messages?**
No. Designs work on every plan. Free adds a "Powered by TicketCord.com" line, the self-help menu and applications need Basic or higher, and custom footer text needs Enterprise.
