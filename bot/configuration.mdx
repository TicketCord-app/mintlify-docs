...
| 📝 Ticket Logs | Category holding `#ticket-logs` and `#ticket-transcripts` |
| Ticket Admin | Role with full ticket management access (**TicketCord Admin** on the Free plan) |
| Ticket Support | Role with basic ticket access (**TicketCord Support** on the Free plan) |
...
- **Create Anyway (New Names)** - Creates a second set with TicketCord-prefixed names (🎫 TicketCord Tickets, 📁 TicketCord Closed, 📝 TicketCord Logs)
- **Use Existing (Recommended)** - Skip creation and configure existing resources on the dashboard
- **Cancel** - Stop setup without changing anything

## Step 2: Dashboard Configuration

After running `/setup`, go to [ticketcord.com/dashboard](https://ticketcord.com/dashboard) to complete the setup:
...
| Auto-reminder | Post a reminder in tickets that have gone quiet, optionally tagging the creator |
