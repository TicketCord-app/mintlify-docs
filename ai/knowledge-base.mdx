---
title: "Knowledge Base"
description: "Teach your bot to automatically answer questions from your documentation"
---

The Knowledge Base lets you connect your documentation, FAQs, or website to your bot. When users open tickets, your bot automatically suggests relevant answers based on what you've taught it.

<Info>
  Knowledge Base is available on **Pro** and **Enterprise**. [Upgrade your plan](https://ticketcord.com/pricing) to access it.
</Info>

## How It Works

1. **Add Sources** - Connect your website, documentation, or FAQ pages
2. **Bot Learns** - We scan and understand your content automatically
3. **Auto-Suggest** - When users ask questions, your bot suggests answers from your sources

Your support team doesn't need to type the same answers repeatedly. The bot handles common questions while staff focus on complex issues.

## Getting Started

<Steps>
  <Step title="Enable Knowledge Base">
    Go to **Dashboard → Bots**, pick your bot, choose the server, and open **AI & Knowledge** → **Knowledge Base**

    Toggle the switch to enable it.
  </Step>
  <Step title="Add a Source">
    Click **Add Knowledge Source** and enter:
    - **URL** - Link to your documentation page
    - **Title** - A name to identify this source
    - **Type** - Single page, documentation site, Discord channel, pasted text, or a GitHub repository (Enterprise)

    Click **Save**. TicketCord automatically queues the first scan.
  </Step>
  <Step title="Test It">
    Open a test ticket and ask a question that your documentation answers.

    Your bot should suggest an answer within a few seconds.
  </Step>
</Steps>

<Card title="Connect Claude Code or Codex" icon="plug" href="/ai/mcp">
  Manage your tickets and Knowledge Base from an AI tool using a secure MCP connection.
</Card>

## Source Types

You can add different types of content sources:

| Type | Best For | How It Works |
|------|----------|--------------|
| **Single Page** | FAQ pages, specific guides | Scans one page only |
| **Documentation Site** | Full documentation | Crawls multiple pages (up to 100 on Pro, 500 on Enterprise) |
| **Discord Channel** | Announcements, rules | Indexes messages from a channel |
| **Text** | Internal notes, policies | Indexes content you paste (up to 50,000 characters) |
| **GitHub Repository** | Code and docs in a repo (Enterprise) | Syncs files from a connected repository |

### Single Page

Use this for standalone pages like:
- FAQ pages
- Pricing pages
- Individual guides or tutorials

The bot scans just that one page and learns from its content.

### Documentation Site

Use this for complete documentation sites. The bot will:
1. Start from your main page
2. Follow links to discover more pages
3. Scan up to 100 pages automatically (500 on Enterprise)

<Tip>
  **Pages to Read** sets how many pages are scanned (1, 5, 10, 15, 20, 25, 50, 75, or 100; up to 500 on Enterprise). 1 means the start page only.
</Tip>

### Discord Channel

Use this to teach your bot from existing Discord content:
- Announcement channels
- Rules channels
- FAQ channels

The bot reads messages from the channel and learns from them.

<Warning>
  Only use public channels. The bot respects Discord permissions and won't access private channels.
</Warning>

## Source Settings

Each source has settings you can customize:

### Auto-refresh

How often the bot checks for changes:

| Frequency | When to Use |
|-----------|-------------|
| **Manual** | Content rarely changes |
| **Realtime** | Discord channels and connected repositories that should stay in sync |
| **Daily** | Active documentation with frequent updates |
| **Weekly** | Moderate update frequency |
| **Monthly** | Stable content that rarely changes |

<Info>
  You can always re-index a source by clicking **Refresh**. A source can be refreshed once every 24 hours.
</Info>

## AI Settings

Customize how your bot responds to questions.

### Response Style

Choose how detailed AI suggestions should be:

| Style | Description |
|-------|-------------|
| **Concise** | Short answers that get straight to the point |
| **Balanced** | Mix of context and actionable steps (recommended) |
| **Helpful** | Detailed explanations with extra context |

### Match Strictness

The minimum confidence (10-100%) required before showing a suggestion:

- **70%+** - Only shows suggestions the AI is confident about
- **50-70%** - Shows more suggestions, but some may be less accurate (the default is 50%)
- **Below 50%** - Not recommended, may show irrelevant suggestions

<Tip>
  Start with the default of 50% and adjust based on your results. Raise it if suggestions are often wrong, lower it if the bot rarely answers.
</Tip>

## Behavior Settings

Control when and how the bot provides suggestions.

### Stop on Staff Response

When enabled (off by default), the bot stops suggesting answers once a staff member replies. This prevents the AI from interrupting staff conversations.

### Respect Staff Presence

When enabled, the bot backs off if staff members are actively responding to a ticket. It waits 5 minutes after the last staff message before suggesting again.

### Outside Business Hours Only

Only provide AI suggestions when no staff are available (outside your configured business hours). Useful if you want humans to handle tickets during work hours.

## Feedback System

The feedback system helps your bot learn and improve over time.

### How It Works

When your bot suggests an answer, users see thumbs up/down buttons:
- 👍 **Helpful** - The answer was useful
- 👎 **Not Helpful** - The answer wasn't relevant or correct

The bot uses this feedback to improve future suggestions.

### Settings

| Setting | Description |
|---------|-------------|
| **Enable Feedback** | Show vote buttons on suggestions |
| **Staff Only Voting** | Only count votes from staff members |
| **Self-Learning** | Automatically adjust content ranking based on votes |

### Self-Learning

When enabled, the bot automatically:
- **Boosts** content that gets positive feedback (shown more often)
- **Penalizes** content that gets negative feedback (shown less often)

This happens after a minimum number of votes (default: 3) to avoid acting on outliers.

## Escalation

When the AI doesn't have a good answer, it can automatically escalate to your team.

### Auto-Escalation

When enabled, the bot will:
1. Detect when it can't find a confident answer
2. Notify your staff team
3. Let users know a human will help them

### Human Assistance Detection

The bot understands when users want to talk to a human. Phrases like:
- "I want to speak to someone"
- "Can I talk to a real person?"
- "Human please"

Will trigger immediate escalation to your team.

## Learning from Closed Tickets

The bot learns from your own support conversations automatically. There is no command to run.

### How It Works

1. When a ticket closes, the bot reviews the conversation
2. Answers that staff gave are extracted as question-and-answer pairs
3. Pairs above your confidence threshold are added to the Knowledge Base
4. Pairs below the threshold wait for approval in the **Q&A** tab of the Knowledge Base

<Info>
  Learning is available on Pro and Enterprise plans and is on by default for newly configured knowledge bases. Manage it under **Knowledge Base** → **Learning**: the auto-approve threshold (default 70%), the daily cap on auto-approved entries (default 50), notifications with an **Undo** button, and automatic merging of near-duplicate entries.
</Info>

### Reviewing What Was Learned

Open the Knowledge Base **Q&A** tab to approve, edit, or reject suggested pairs. Auto-approved pairs can be undone from the notification the bot posts.

## Limits by Plan

| Feature | Pro | Enterprise |
|---------|-----|------------|
| Knowledge Base Sources | 2 | 15 |
| AI Tokens/Month | 10M | 30M |
| Auto-Updates | ✅ | ✅ |
| Feedback Analytics | ✅ | ✅ |

<Card title="Upgrade Your Plan" icon="rocket" href="https://ticketcord.com/pricing">
  Get more sources and AI tokens with Enterprise
</Card>

## Troubleshooting

### Bot Not Suggesting Answers

1. **Check if enabled** - Make sure Knowledge Base is toggled on
2. **Check sources** - At least one source should be indexed (green status)
3. **Lower confidence** - Try lowering the confidence threshold to 50%
4. **Check triggers** - Make sure the minimum message length is not filtering out real questions

### Indexing Failed

1. **Check the URL** - Make sure the page is publicly accessible
2. **Try again** - Sometimes pages are temporarily unavailable
3. **Check page size** - Very large pages (50,000+ characters) may be trimmed

### Wrong Answers

1. **Check source content** - Make sure your documentation is accurate
2. **Add better sources** - More specific documentation helps
3. **Use categories** - Limit which tickets can use each source
4. **Enable feedback** - Let users vote to improve suggestions

<Card title="Need Help?" icon="headset" href="https://ticketcord.com/discord">
  Join our Discord server for support
</Card>
