...
<Warning>
  The Web Chat Widget is **not currently available**. It is disabled on all plans while its future is decided; the widget script returns a 404 and the settings below have no effect. This page is kept for reference only.
</Warning>
...
```html
<script
  src="https://ticketcord.com/api/webchat/widget.js"
...
```
script-src 'self' https://ticketcord.com;
connect-src 'self' https://ticketcord.com;
style-src 'self' 'unsafe-inline';
img-src 'self' https://ticketcord.com https://cdn.discordapp.com;
```
...
|-----------|----------------|
| `script-src https://ticketcord.com` | The widget script itself |
| `connect-src https://ticketcord.com` | API calls (config, session, messages) and the SSE stream |
...
const nextConfig = {
  images: {
    remotePatterns: [
      { protocol: 'https', hostname: 'ticketcord.com' },
...
<AccordionGroup>
  ...
  <Accordion title="Widget loads but messages don't send">
    ...
    - Look for rate limit errors in the console — the default is 5 messages per minute per visitor (configurable from 1 to 60 in the dashboard).
  </Accordion>
</AccordionGroup>
