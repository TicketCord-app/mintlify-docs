...
<Info>
  Webhooks are an **Enterprise** feature. [Upgrade your plan](https://ticketcord.com/pricing) to access webhooks.
</Info>
...
- **Attempts**: 1 delivery plus up to 3 retries by default (configurable per webhook, 0 to 5)
- **Backoff**: Exponential, 1s, 2s, 4s by default (initial delay 100 ms to 10 s, multiplier 1 to 5, capped at 5 minutes)
- **429 responses**: not counted as a failed attempt; delivery pauses for the `Retry-After` value (default 60 seconds) and is requeued
- **Success codes**: Any 2xx response is considered successful
- **Timeout**: 30 seconds per request
- **Limits**: 20 webhooks per server, 60 deliveries per minute per webhook, HTTPS endpoints only (no private addresses)
- **Logs**: Delivery logs are kept for 30 days
...
Use the **Test Fire** button in the TicketCord dashboard to send a test delivery to your endpoint. It is sent under the first event type the webhook subscribes to (so the event header will be, for example, `ticket.created`), with a test payload:

```json
{
  "data": {
    "test": true,
    "message": "This is a test webhook delivery from TicketCord",
