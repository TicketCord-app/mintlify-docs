...
**What happens:** the saved response is posted in the ticket. `{{user}}` is replaced with the creator's mention and `{{ticketNumber}}` with the ticket number; the older `{user}` and `{ticketNumber}` forms still work.

Create responses in the dashboard under **Canned Responses**, each with a shortcut and content.
