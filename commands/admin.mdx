...
**Permission:** Administrator, or Manage Channels + Manage Roles
...
<Warning>
  The `/setup` command only creates Discord resources. You **must** complete configuration on the [dashboard](https://ticketcord.com/dashboard) for your bot to work.
</Warning>
...
<Steps>
  <Step title="Go to the dashboard">
    Visit [ticketcord.com/dashboard](https://ticketcord.com/dashboard)
  </Step>
...
|--------|------|----------|-------------|
| `name` | Choice | Yes | Name of the saved embed |
...
| `reason` | Text | No | Detailed reason for request |
| `urgency` | Choice | No | How urgent is this |
| `amount` | Number | No | Amount involved (for refunds) |
...
<Info>
  Configure approval roles and notification channels in your dashboard under **Approvals**.
</Info>
...
| Option | Type | Required | Description |
|--------|------|----------|-------------|
| `type` | Choice | No | What to purge (defaults to `all`) |

**Type choices:**
- `all` - Delete all tickets
- `open` - Delete open tickets only
...
Configure automatic closure of inactive tickets.
...
| Option | Type | Required | Description |
|--------|------|----------|-------------|
| `time` | Text | Yes | Inactivity period such as `30M`, `2H`, or `1D`, or `off` to disable |

**Permission:** Administrator  
**Plan:** Basic or higher

**Limits:** minimum 15 minutes, maximum 30 days.
...
Open an interactive configuration panel for the ticket system inside Discord.
...
The panel lets you review and toggle core settings without leaving Discord. Some settings are managed from the dashboard only; the panel tells you when that is the case.

---

## /diagnose

Check the bot's permissions and configuration for the current server.
...
**What it checks:**
- Whether the bot has the permissions it needs (View Channel, Send Messages, Manage Channels, and more)
- Whether the ticket categories and staff roles in your configuration still exist
- Common setup problems, with suggested fixes
