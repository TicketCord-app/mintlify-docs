...
<Steps>
  ...
  <Step title="Create TicketCord Bot">
    Sign up and create your bot at [ticketcord.com](https://ticketcord.com)
  </Step>
...
**Panels → Embeds:**
1. Open the server configuration (Dashboard → Bots → your bot → Servers → the server) and go to **Panels & Embeds**
2. Create new embed with similar design
3. Add buttons for ticket creation
4. Deploy to your support channel

**Teams → Staff Roles:**
1. In the server configuration, go to **Staff**
2. Add your existing Discord roles
3. Set permissions for each role

**Auto-close → Automation:**
1. In the server configuration, go to **Auto-Close & Reminders**
2. Enable auto-close
...
| ModMail | TicketCord |
|---------|------------|
| DM-based tickets | Channel tickets or ModMail (DM-based) |
...
|---------|---------------------|
| Welcome message | Server configuration → Welcome Messages |
| Staff roles | Server configuration → Staff |
| Categories | Server configuration → Structure |
| Close settings | Server configuration → Closing |
| Transcripts | Server configuration → Transcripts |
| Auto-close | Server configuration → Auto-Close & Reminders |
| Custom embeds | Server configuration → Panels & Embeds |
